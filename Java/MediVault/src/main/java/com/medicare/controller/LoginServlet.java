...
public class LoginServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        if ("true".equals(rememberMe)) {
            // 7 ngày
            com.medicare.controller.AuthFilter.writeAdminCookieLong(resp, account.getAccountId(), 7 * 24 * 60 * 60);
        } else {
            // Session-only cookie (8 tiếng)
            com.medicare.controller.AuthFilter.writeAdminCookie(resp, account.getAccountId());
        }
