...
public final class BarcodeService {
    ...
    public static String medicineJson(Medicines m) {
        ...
        return "{\"id\":" + m.getMedicineId()
                ...
                + ",\"genericName\":" + jstr(m.getGenericName())
                + ",\"code\":" + jstr(m.getMedicineCode())
                + ",\"barcode\":" + jstr(m.getBarcode())
                + ",\"unit\":" + jstr(m.getUnit())
                + ",\"price\":" + (m.getSellingPrice() != null ? m.getSellingPrice().toPlainString() : "0")
                + ",\"totalStock\":" + m.getTotalStock()
                + ",\"nearestBatchNo\":" + jstr(m.getNearestBatchNo())
                + ",\"nearestExpiry\":" + jstr(m.getNearestExpiry())
                + ",\"categoryId\":" + (m.getCategoryId() != null ? m.getCategoryId() : "null")
                + ",\"categoryName\":" + jstr(m.getCategoryName())
                + ",\"manufacturerId\":" + (m.getManufacturerId() != null ? m.getManufacturerId() : "null")
                + ",\"manufacturerName\":" + jstr(m.getManufacturerName())
                + ",\"shelfId\":" + (m.getShelfId() != null ? m.getShelfId() : "null")
                + ",\"shelfName\":" + jstr(m.getShelfName())
