  ...
  <%@ taglib prefix="c" uri="jakarta.tags.core" %>
    <%@ taglib prefix="fmt" uri="jakarta.tags.fmt" %>
      <%@ taglib prefix="fn" uri="jakarta.tags.functions" %>
        <% com.medicare.entity.Account acc=(com.medicare.entity.Account) session.getAttribute("staffAccount"); boolean
          isLoggedIn=(acc !=null && acc.getRoleId() !=1); String fullName=isLoggedIn ? (acc.getFullName() !=null ?
          acc.getFullName() : acc.getUsername()) : "Dược sĩ" ; String initials=fullName.length()>= 2
            ...
            <style>
              ...
              *::after {
              ...
              .sb-label {
                ...
                white-space: nowrap;
                opacity: 0;
                transition: opacity .14s .06s
              }

              .sb-btn:hover .sb-label,
              .sb-btn.active .sb-label {
                color: #fff
              }
              ...
              .center {
                margin-left: var(--sw);
                width: calc(100vw - var(--sw) - var(--rw));
                height: 100vh;
                display: flex;
                flex-direction: column;
                background: var(--surface)
              ...
              .pay-method-card.active .pmc-label {
                color: var(--blue)
              }
              ...
              .btn-checkout:hover:not(:disabled) {
                box-shadow: 0 6px 22px rgba(5, 150, 105, .45);
              ...
              @keyframes ringPulse {
                ...
                0%,
                100% {
                  box-shadow: 0 0 0 2000px rgba(0, 0, 0, .3), 0 0 0 0 rgba(59, 130, 246, .4)
                }

                50% {
                  box-shadow: 0 0 0 2000px rgba(0, 0, 0, .3), 0 0 0 12px rgba(59, 130, 246, 0)
                }
              }
              ...
              .staff-action-wrap.open .sa-arrow {
              ...
              .staff-action-menu button.sam-danger {
                color: #dc2626
              }

              .staff-action-menu button.sam-danger:hover {
                background: #fef2f2
              }
              ...
              .qrm-panel {
                ...
                align-items: center;
                gap: 12px;
                animation: qrmPop .22s cubic-bezier(.34, 1.56, .64, 1)
              }
              ...
              .qrm-dot {
                ...
                animation: qrmDot 1.4s ease infinite
              }
              ...
              .qrm-success-box.show {
                display: flex
              }

              .qrm-success-icon {
                font-size: 52px;
                animation: qrmBounce .35s cubic-bezier(.34, 1.56, .64, 1)
              }
              ...
              .esr-confirm:hover:not(:disabled) {
                background: linear-gradient(135deg, #B91C1C, #991B1B)
              }

              .esr-confirm:disabled {
                background: #CBD5E1;
                cursor: not-allowed
              }
            </style>
                ...
                <div class="sb-checkin-wrap">
                  <% String checkinAction=isLoggedIn ? "toggleCheckinPanel()" : "openFaceModal()" ; %>
                    <button class="sb-btn" id="checkinBtn" onclick="<%= checkinAction %>">
                      ...
                      <span class="sb-label">
                        <%= isLoggedIn ? fullName : "Điểm danh" %>
                      </span>
                      <span class="sb-tip">
                        <%= isLoggedIn ? "Ca làm / " + fullName : "Điểm danh nhân viên" %>
                      </span>
                    </button>
           ...
           padding:15px;box-shadow:0 8px 32px rgba(0,0,0,.4);z-index:9999">
                      <% if (isLoggedIn) { %>
                        <div style="display:flex;align-items:center;gap:9px;margin-bottom:11px">
                          <div
                            style="width:34px;height:34px;background:linear-gradient(135deg,#3f83f8,#1a56db);border-radius:9px;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:800;color:#fff">
                            <%= initials %>
                          </div>
                          <div>
                            ...
                            <div style="font-size:10.5px;color:rgba(255,255,255,.45)">Đang ca làm việc</div>
                          </div>
                        </div>
                        <a href="<%= ctx %>/staff-dashboard"
                          style="display:block;padding:8px 11px;background:rgba(255,255,255,.1);border-radius:8px;color:#93c5fd;font-size:12.5px;font-weight:750;text-decoration:none;margin-bottom:5px;text-align:center">📅
                          Xem lịch ca</a>
                        <button onclick="openMyInvModal();toggleCheckinPanel();"
                          style="display:block;width:100%;padding:8px 11px;background:rgba(255,255,255,.1);border-radius:8px;color:#a5f3fc;font-size:12.5px;font-weight:750;text-align:center;border:none;cursor:pointer;font-family:inherit;margin-bottom:5px">🧾
                          Hóa đơn của tôi</button>
                        <button onclick="openOpenCashModal();toggleCheckinPanel();"
                          style="display:block;width:100%;padding:8px 11px;background:rgba(255,255,255,.1);border-radius:8px;color:#86efac;font-size:12.5px;font-weight:750;text-align:center;border:none;cursor:pointer;font-family:inherit;margin-bottom:5px">💵
                          Khai báo tiền đầu ca (tùy chọn)</button>
                        <button onclick="openEndShiftModal();toggleCheckinPanel();"
                          style="display:block;width:100%;padding:8px 11px;background:rgba(239,68,68,.15);border-radius:8px;color:#fca5a5;font-size:12.5px;font-weight:750;text-align:center;border:none;cursor:pointer;font-family:inherit">⏻
                          Kết thúc ca</button>
                        <% } else { %>
                          <div style="font-size:11.5px;color:rgba(255,255,255,.5);margin-bottom:11px">Điểm danh để ghi
                            nhận doanh số theo nhân viên</div>
                          <!-- Nút điểm danh khuôn mặt -->
                          <button onclick="openFaceModal();toggleCheckinPanel();"
                            style="width:100%;padding:10px;background:linear-gradient(135deg,#059669,#047857);border:none;border-radius:8px;color:#fff;font-size:13px;font-weight:750;cursor:pointer;font-family:inherit;margin-bottom:6px;display:flex;align-items:center;justify-content:center;gap:6px">
                            📷 Điểm danh khuôn mặt
                          </button>
                          <div style="font-size:10.5px;color:rgba(255,255,255,.25);margin-top:6px;text-align:center">POS
                            hoạt động bình thường khi chưa đăng nhập</div>
                          <% } %>
                    </div>
                </div>
              </div>
            ...
            <div class="center">
              ...
              <div class="topbar">
                ...
                <div class="station-badge" onclick="openStationModal()" id="stationBadge" title="Chọn quầy POS">
                  ...
                  <div>
                    <div class="station-label" id="stationLabel">
                      <%= stationLabel %>
                    </div>
                    <div class="station-staff" id="stationStaff">
                      <%= isLoggedIn ? fullName : "Chưa điểm danh" %>
                    </div>
                  </div>
                </div>
              ...
              <div class="cat-bar" id="catBar">
                ...
                <c:forEach var="cat" items="${categories}">
                  <button class="cat-tab" data-cat="${cat.categoryId}" onclick="filterCat(this,${cat.categoryId})">
              ...
              <% if (!isLoggedIn) { %>
                <div id="checkinBanner"
                  style="padding:10px 16px;background:linear-gradient(90deg,#fffbeb,#fef3c7);border-bottom:2px solid #f59e0b;display:flex;align-items:center;gap:12px;flex-shrink:0">
                  <span style="font-size:20px;flex-shrink:0">⏰</span>
                  <div style="flex:1;min-width:0">
                    <div style="font-size:13px;font-weight:750;color:#92400e">Chưa điểm danh ca làm</div>
                    <div style="font-size:11.5px;color:#b45309;margin-top:1px">Vui lòng điểm danh để ghi nhận doanh số
                    </div>
                  </div>
                  <button onclick="openFaceModal()"
                    style="padding:8px 18px;background:#f59e0b;border:none;border-radius:9px;color:#fff;font-size:13px;font-weight:750;cursor:pointer;font-family:inherit;white-space:nowrap;flex-shrink:0;display:flex;align-items:center;gap:6px">
                    📷 Điểm danh ngay
                  </button>
                </div>
                <% } %>
                  ...
                  <%-- Modal: Khai báo tiền đầu ca (mở ca) --%>
                    <div id="openCashModal"
                      style="display:none;position:fixed;inset:0;z-index:9600;background:rgba(11,22,40,.55);align-items:center;justify-content:center;padding:20px">
                      <div
                        style="background:#fff;border-radius:18px;max-width:400px;width:100%;padding:28px;box-shadow:0 24px 70px rgba(0,0,0,.35)">
                        <div style="display:flex;align-items:center;gap:10px;margin-bottom:6px">
                          <span style="font-size:26px">💵</span>
                          <h3 style="margin:0;font-size:18px;font-weight:800;color:#0f172a">Khai báo tiền đầu ca</h3>
                        </div>
                        <p style="font-size:12.5px;color:#64748b;margin:0 0 16px;line-height:1.5">Đếm tiền mặt hiện có
                          trong két và nhập số tiền để mở ca. Số này dùng đối soát khi kết ca.</p>
                        <label
                          style="font-size:12.5px;font-weight:700;color:#334155;display:block;margin-bottom:6px">Tiền
                          mặt trong két (đ) <span style="color:#dc2626">*</span></label>
                        ...
                        <button type="button" id="openShiftBtn" onclick="confirmOpenShift()"
                          style="width:100%;margin-top:16px;padding:13px;background:linear-gradient(135deg,#059669,#047857);border:none;border-radius:12px;color:#fff;font-size:15px;font-weight:800;cursor:pointer;font-family:inherit">✓
                          Mở ca &amp; bắt đầu bán hàng</button>
                      </div>
                    </div>
                    ...
                    <%-- Modal: Hóa đơn của tôi (bill trong ca hôm nay của chính nhân viên) --%>
                      <div id="myInvModal"
                        style="display:none;position:fixed;inset:0;z-index:9600;background:rgba(11,22,40,.55);align-items:center;justify-content:center;padding:20px"
                        onclick="if(event.target===this)closeMyInvModal()">
                        <div
                          style="background:#fff;border-radius:18px;max-width:520px;width:100%;max-height:86vh;display:flex;flex-direction:column;box-shadow:0 24px 70px rgba(0,0,0,.35);overflow:hidden">
                          <div
                            style="padding:18px 22px;background:linear-gradient(135deg,#1558A8,#3ABDE0);color:#fff;display:flex;align-items:center;justify-content:space-between">
                            <div>
                            ...
                            <button onclick="closeMyInvModal()"
                              style="background:rgba(255,255,255,.18);border:none;color:#fff;width:32px;height:32px;border-radius:9px;font-size:16px;cursor:pointer">✕</button>
                          </div>
                          <!-- Bộ chọn thời gian -->
                          <div
                            style="padding:10px 18px;border-bottom:1px solid #e2e8f0;background:#f8fafc;display:flex;align-items:center;gap:8px">
                            <label for="myInvRange" style="font-size:13px;font-weight:700;color:#475569">Thời
                              gian:</label>
                            <select id="myInvRange" onchange="openMyInvModal()"
                              style="padding:6px 12px;border:1.5px solid #cbd5e1;border-radius:8px;font-size:13px;font-weight:600;font-family:inherit;color:#1e293b;background:#fff;cursor:pointer">
                      ...
                      <div class="med-grid" id="medGrid">
                        <c:forEach var="m" items="${medicines}">
                          ...
                          <div class="${cardCls}" data-id="${m.medicineId}"
                            data-name="<c:out value='${m.medicineName}' />" data-price="${m.sellingPrice}"
                            data-unit="<c:out value='${m.unit}' />" data-cat="${m.categoryId}"
                            data-rx="${m.prescriptionRequired}" data-dosage="<c:out value='${m.dosage}' />"
                            data-code="<c:out value='${m.medicineCode}' />" data-stock="${mStock}"
                            data-batchno="<c:out value='${mBatch}' />" data-expiry="${mExpiry}"
                            data-minstock="${m.minInventory}" data-generic="<c:out value='${m.genericName}' />"
                            data-contra="<c:out value='${m.contraindications}' />"
                            data-warning="<c:out value='${m.dosageWarning}' />"
                            data-storage="<c:out value='${m.storageConditions}' />" onclick="addToCart(this)">
                            <%-- Top row: stock badge left, Rx/OTC right --%>
                              <div class="mc-top">
                                <span class="${stkCls} mc-stock">${stkLbl}</span>
                                <c:choose>
                                  <c:when test="${m.prescriptionRequired}"><span class="mc-badge mb-rx">Rx</span>
                                  </c:when>
                              ...
                              <%-- Center: medicine type icon --%>
                                <c:choose>
                                  <c:when
                                    test="${fn:contains(m.unit,'Siro') or fn:contains(m.unit,'siro') or fn:contains(m.unit,'Chai') or fn:contains(m.unit,'chai') or fn:contains(m.unit,'ml') or fn:contains(m.unit,'Dung') or fn:contains(m.unit,'dịch')}">
                                    <span class="mc-icon">🍶</span></c:when>
                                  <c:when
                                    test="${fn:contains(m.unit,'Ống') or fn:contains(m.unit,'ống') or fn:contains(m.unit,'Amp')}">
                                    <span class="mc-icon">💉</span></c:when>
                                  <c:when
                                    test="${fn:contains(m.unit,'Gói') or fn:contains(m.unit,'gói') or fn:contains(m.unit,'Túi')}">
                                    <span class="mc-icon">📦</span></c:when>
                                  <c:when
                                    test="${fn:contains(m.unit,'Tuýp') or fn:contains(m.unit,'tuýp') or fn:contains(m.unit,'Kem') or fn:contains(m.unit,'kem') or fn:contains(m.unit,'Gel') or fn:contains(m.unit,'gel')}">
                                    <span class="mc-icon">🧴</span></c:when>
                                ...
                                <%-- Name & unit --%>
                                  ...
                                  <%-- Price bottom --%>
                                    ...
                                    <%-- Info button — bottom right corner --%>
                                      <button class="mc-info-btn"
                                        onclick="event.stopPropagation();showMedInfo(${m.medicineId})"
            ...
            <div class="invoice-panel">
              ...
              <div class="inv-customer">
                ...
                <div class="cust-wrap">
                  <input type="text" id="custPhone" placeholder="Nhập SĐT để tìm khách…" oninput="onCustInput()"
                    autocomplete="off">
              ...
              <div class="inv-items" id="invItems">
                <div class="inv-empty">
                  <div class="ei">🛒</div>
                  <p>Giỏ hàng trống<br><small>Bấm vào thẻ thuốc để thêm</small></p>
                </div>
              </div>
              ...
              <div class="inv-forms">
                  ...
                  <div class="pay-detail-section" id="qrSection">
                    <div class="pdi-row">
                      ...
                      <div class="pdi-body">
                        ...
                        <div class="pdi-sub">Nhấn <b>Thanh toán</b> để tạo mã QR — khách quét và chuyển khoản tự động
                        </div>
                      </div>
                ...
                <div class="cash-section show" id="cashSection">
                  ...
                  <div class="cash-input-row">
                    ...
                    <input type="number" class="f-input cash" id="cashInput" placeholder="Nhập số tiền…" min="0"
                      oninput="calcChange()">
                  </div>
              ...
              <div class="inv-totals">
                ...
                <div class="total-row">
                  <span>Giảm giá (₫)</span>
                  <input type="number" class="f-input discount" id="discountInput" value="0" min="0"
                    oninput="updateTotal()" style="width:80px;height:26px;font-size:12px;padding:0 7px">
                </div>
            ...
            <div class="face-modal" id="faceModal">
              ...
              <div class="fm-panel">
                ...
                <div id="fmLoading" class="fm-loader">
                  <div class="fm-spinner"></div>Đang tải mô hình nhận dạng…
                </div>
                <div id="fmCameraWrap" style="display:none">
                  ...
                  <div class="fm-actions">
                    ...
                    <button class="fm-btn-checkin" id="fmCheckinBtn" onclick="confirmFaceCheckin()">✓ Xác nhận điểm
                      danh</button>
                  </div>
                </div>
              </div>
              ...
              <div class="esr-box">
                ...
                <div class="esr-body">
                  ...
                  <div class="esr-left">
                    ...
                    <div class="esr-note">💡 QR và Thẻ không ảnh hưởng đến két tiền mặt tại quầy. Chỉ tính vào doanh số
                      nhân viên.</div>
                  </div>
                  ...
                  <div class="esr-right">
                    ...
                    <div class="esr-inp-wrap">
                      ...
                      <input type="number" class="esr-inp" id="esrActualInput" placeholder="0" min="0"
                        oninput="calcCashVariance()" autocomplete="off">
                    </div>
                ...
                <div class="esr-foot">
                  ...
                  <button class="esr-confirm" id="esrConfirmBtn" onclick="confirmEndShift()">⏻ Xác nhận &amp; Đóng
                    ca</button>
                </div>
              </div>
            </div>
    ...
    <div class="center print-actions" style="margin-top:8px">
        ...
        <button onclick="window.close()" style="padding:8px 16px;background:#e5e7eb;color:#111;border:none;border-radius:6px;cursor:pointer;font-size:13px;margin-left:6px;font-family:'Plus Jakarta Sans',sans-serif">✕ Đóng</button>
              ...
              function confirmStation() {
                ...
                updateStationUI();
                closeStationModal();
                showToast('✓ Đã chuyển sang Quầy ' + station, 'ok');
              }

              function updateStationUI() {
                const lbl = document.getElementById('stationLabel');
                if (lbl) lbl.textContent = currentStation > 0 ? 'Quầy ' + currentStation : 'Chọn quầy';
              }
              ...
              async function confirmFaceCheckin() {
                ...
                try {
                  ...
                  if (data.ok) {
                    ...
                    showToast('⚠️ Ca của ' + (data.name || '') + ' được xếp ở Quầy ' + data.correctStation, 'err');
                    btn.disabled = false;
