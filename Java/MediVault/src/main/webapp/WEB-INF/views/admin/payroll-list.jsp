...
<title>Bảng lương — medicare</title>
...
<script src="${pageContext.request.contextPath}/js/csrf.js"></script>
  ...
  <header class="topbar"><div style="font-size:15px">💰</div><span class="topbar-title">Bảng lương</span>
    <div class="topbar-right">
      <form method="get" action="${pageContext.request.contextPath}/payroll" style="display:flex;gap:8px;align-items:center">
        <input type="hidden" name="month" id="hMonth" value="${month}">
        <div class="cdd" id="cddMonth" style="font-size:13px">
          <div class="cdd-btn" onclick="toggleCdd('cddMonth')" style="padding:6px 10px">
            <span class="cdd-label">${month}</span>
            <span class="cdd-arrow">▼</span>
          </div>
          <div class="cdd-menu">
            <c:forEach begin="1" end="12" var="m">
              <div class="cdd-opt ${m==month?'active':''}" data-val="${m}" onclick="pickCdd('cddMonth','hMonth',this,false)">${m}</div>
            </c:forEach>
          </div>
        </div>
        <input type="number" name="year" value="${year}" style="padding:6px 10px;border:1.5px solid var(--border);border-radius:8px;font-family:inherit;font-size:13px;width:80px">
      ...
      <a href="${pageContext.request.contextPath}/payroll?action=generate&month=${month}&year=${year}"
         onclick="return confirm('Tạo bảng lương tháng ${month}/${year} cho tất cả nhân viên?')"
         style="padding:7px 16px;background:var(--green);color:#fff;border-radius:8px;font-size:13px;font-weight:750;text-decoration:none">
        ⚡ Tính lương tháng
      </a>
    </div>
  </header>
  <div class="content">
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:20px">
      <div style="background:var(--white);border:1px solid var(--border);border-radius:var(--radius);padding:14px 18px">
        <div style="font-size:11px;font-weight:750;color:var(--muted);text-transform:uppercase;letter-spacing:.5px">Tổng chi lương</div>
        <div style="font-size:20px;font-weight:800;color:var(--ink);margin-top:4px"><c:if test="${not empty totalNet}"><fmt:formatNumber value="${totalNet}" type="number" maxFractionDigits="0"/>đ</c:if><c:if test="${empty totalNet}">0đ</c:if></div>
      </div>
      <div style="background:var(--white);border:1px solid var(--border);border-radius:var(--radius);padding:14px 18px">
        <div style="font-size:11px;font-weight:750;color:var(--muted);text-transform:uppercase;letter-spacing:.5px">Draft</div>
        <div style="font-size:20px;font-weight:800;color:var(--amber);margin-top:4px">${draftCount}</div>
      </div>
      <div style="background:var(--white);border:1px solid var(--border);border-radius:var(--radius);padding:14px 18px">
        <div style="font-size:11px;font-weight:750;color:var(--muted);text-transform:uppercase;letter-spacing:.5px">Đã xác nhận</div>
        <div style="font-size:20px;font-weight:800;color:var(--blue);margin-top:4px">${confirmedCount}</div>
      </div>
      <div style="background:var(--white);border:1px solid var(--border);border-radius:var(--radius);padding:14px 18px">
        <div style="font-size:11px;font-weight:750;color:var(--muted);text-transform:uppercase;letter-spacing:.5px">Đã trả</div>
        <div style="font-size:20px;font-weight:800;color:var(--green);margin-top:4px">${paidCount}</div>
      </div>
    </div>
    <div class="table-card">
      <div class="table-card-head"><h2>💰 Bảng lương tháng ${month}/${year}</h2><span class="tc-sub">${fn:length(payrolls)} nhân viên</span></div>
      <c:choose>
        <c:when test="${empty payrolls}"><div class="empty-box"><div style="font-size:36px;margin-bottom:10px">💰</div><p>Chưa có bảng lương tháng này. Nhấn "Tính lương tháng" để tạo.</p></div></c:when>
        <c:otherwise>
          <table>
            <thead><tr><th>Nhân viên</th><th>Ngày làm</th><th>Giờ thực</th><th>Lương cơ bản</th><th>OT</th><th>Phụ cấp</th><th>Thưởng</th><th>Khấu trừ</th><th style="color:var(--green)">Thực nhận</th><th>Trạng thái</th><th></th></tr></thead>
            <tbody>
              <c:forEach var="p" items="${payrolls}">
                <tr>
                  ...
                  <td style="font-weight:800;font-size:14px;color:var(--green)"><c:choose><c:when test="${not empty p.netSalary}"><fmt:formatNumber value="${p.netSalary}" type="number" maxFractionDigits="0"/>đ</c:when><c:otherwise>0đ</c:otherwise></c:choose></td>
                  <td>
                  ...
                  <td><a href="${pageContext.request.contextPath}/payroll?action=detail&id=${p.payrollId}" class="btn-sm btn-primary">Chi tiết</a></td>
                </tr>
              </c:forEach>
            </tbody>
          </table>
