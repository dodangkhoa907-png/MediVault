...
.btn-primary{display:inline-flex;align-items:center;gap:7px;padding:10px 18px;background:linear-gradient(135deg,var(--blue),#0D3F85);color:#fff;border:none;border-radius:10px;font-family:'Plus Jakarta Sans',sans-serif;font-size:13px;font-weight:750;cursor:pointer;text-decoration:none}
...
.po-code{font-family:monospace;font-weight:750;color:var(--blue)}
.btn-detail{display:inline-flex;align-items:center;gap:5px;padding:5px 11px;background:#EFF6FF;color:var(--blue);border:1.5px solid #BFDBFE;border-radius:7px;font-size:12px;font-weight:750;text-decoration:none}
          ...
          <tbody>
            ...
            <c:forEach var="po" items="${pos}">
              <tr onclick="location.href='${pageContext.request.contextPath}/purchase-orders?action=detail&id=${po.poId}'">
                ...
                <td style="font-weight:750">${supplierMap[po.supplierId] != null ? supplierMap[po.supplierId].supplierName : 'NCC #' += po.supplierId}</td>
