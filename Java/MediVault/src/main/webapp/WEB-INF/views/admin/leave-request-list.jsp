...
<title>Danh sách đơn nghỉ — medicare</title>
...
<script src="${pageContext.request.contextPath}/js/csrf.js"></script>
  ...
  <header class="topbar"><div style="font-size:15px">🏖️</div><span class="topbar-title">Đơn xin nghỉ</span>
    <div class="topbar-right">
      <form method="get" action="${pageContext.request.contextPath}/leave-requests" style="display:flex;gap:8px;align-items:center">
      ...
      <a href="${pageContext.request.contextPath}/leave-requests?action=pending" style="padding:7px 16px;background:#FEF3C7;color:#92400E;border-radius:8px;font-size:12.5px;font-weight:750;text-decoration:none">
  ...
  <div class="content">
    <div class="table-card">
      <div class="table-card-head"><h2>📋 Đơn nghỉ tháng ${month}/${year}</h2><span class="tc-sub">${fn:length(leaves)} đơn</span></div>
      <c:choose>
        <c:when test="${empty leaves}"><div class="empty-box"><p>Không có đơn nghỉ tháng này.</p></div></c:when>
        <c:otherwise>
          <table>
            <thead><tr><th>Nhân viên</th><th>Ngày nghỉ</th><th>Loại</th><th>Trạng thái</th><th>Lý do</th><th>Duyệt bởi</th><th>Khấu trừ</th></tr></thead>
            <tbody>
              <c:forEach var="lr" items="${leaves}">
                <tr>
                  <td><strong>${lr.staffName}</strong></td>
                  <td>${lr.leaveDate}</td>
                  <td><span class="badge badge-${fn:toLowerCase(lr.leaveType)}">${lr.leaveType=='ANNUAL'?'Phép năm':lr.leaveType=='SICK'?'Ốm':lr.leaveType=='UNPAID'?'Không lương':'Đột xuất'}</span></td>
                  ...
                  <td style="font-weight:750;color:var(--red)">${lr.deductAmount!=null && lr.deductAmount.compareTo(java.math.BigDecimal.ZERO)>0 ? lr.deductAmount.toPlainString() += 'đ' : '—'}</td>
                </tr>
              </c:forEach>
            </tbody>
          </table>
